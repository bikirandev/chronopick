...
import { ChronoPickMode, SelectedDateType, DateRange, ChronoPickProps, CalendarView } from '../../../types';
import {
  formatDate,
  getDaysInMonth,
  getFirstDayOfMonth,
...
import { DEFAULT_DATE_FORMAT, DEFAULT_TIME_FORMAT, YEARS_PER_VIEW, MONTH_NAMES_SHORT } from '../utils/constants';
...
export const useChronoPickCore = (params: UseChronoPickCoreParams) => {
  ...
  const effectiveDateFormat = enableTime ? `${dateFormat} ${DEFAULT_TIME_FORMAT}` : dateFormat;
  ...
  const getInitialDateForView = useCallback((): Date => {
    ...
    if (mode === ChronoPickMode.Single && value instanceof Date) candidateDate = value;
    if (mode === ChronoPickMode.Multiple && Array.isArray(value) && value.length > 0) candidateDate = value[0];
    if (mode === ChronoPickMode.Range && typeof value === 'object' && value && (value as DateRange).from) candidateDate = (value as DateRange).from as Date;
    ...
    return getFirstFocusableDate(candidateDate, minDate, maxDate, disabledDates, 0);
  ...
  const [currentMonthDate, setCurrentMonthDate] = useState<Date>(getInitialDateForView());
  /** State: The current view of the calendar (Days, Months, or Years). */
  const [currentView, setCurrentView] = useState<CalendarView>(CalendarView.Days);
  ...
  const [focusedDate, setFocusedDateState] = useState<Date>(getInitialDateForView());
  ...
  useEffect(() => {
    ...
    if (enableTime) {
      ...
      if (value instanceof Date) { // Single mode with a date
        timeSourceDate = value;
      } else if (mode === ChronoPickMode.Range && (value as DateRange)?.from) { // Range mode, use 'from' if available
  ...
  useEffect(() => {
    if (isPickerVisible) {
      ...
      const focusableDate = getFirstFocusableDate(initialFocusDateCandidate, minDate, maxDate, disabledDates, 0);
      ...
      if (focusableDate.getFullYear() !== currentMonthDate.getFullYear() || 
          focusableDate.getMonth() !== currentMonthDate.getMonth()) {
        setCurrentMonthDate(new Date(focusableDate.getFullYear(), focusableDate.getMonth(), 1));
      }
    }
  }, [isPickerVisible, value, mode, minDate, maxDate, disabledDates, getInitialDateForView]);
  ...
  const setFocusedDate = (date: Date) => {
    ...
    const clampedDate = getFirstFocusableDate(date, minDate, maxDate, disabledDates, 0); 
    ...
    if (clampedDate.getMonth() !== currentMonthDate.getMonth() || clampedDate.getFullYear() !== currentMonthDate.getFullYear()) {
      setCurrentMonthDate(new Date(clampedDate.getFullYear(), clampedDate.getMonth(), 1));
    }
  };
  ...
  const handleMonthChange = useCallback((offset: number) => {
    ...
    const newBaseDate = currentView === CalendarView.Days ? focusedDate : currentMonthDate;
    ...
    const daysInNewMonth = new Date(newCandidateDate.getFullYear(), newCandidateDate.getMonth() + 1, 0).getDate();
    if (dayToFocus > daysInNewMonth) {
        dayToFocus = daysInNewMonth; // Clamp to last day of new month
    }

    const idealFocusTargetInNewMonth = new Date(newCandidateDate.getFullYear(), newCandidateDate.getMonth(), dayToFocus);
    // Find the first actually focusable date at or after/before the ideal target in the new month
    const focusTarget = getFirstFocusableDate(idealFocusTargetInNewMonth, minDate, maxDate, disabledDates, offset > 0 ? 1 : -1);
    
    setFocusedDateState(focusTarget); // Update focused date
    setCurrentMonthDate(new Date(focusTarget.getFullYear(), focusTarget.getMonth(), 1)); // Update calendar view
  }, [focusedDate, currentMonthDate, currentView, minDate, maxDate, disabledDates]);
  ...
  const handleYearChange = useCallback((offset: number) => {
    ...
    let dayToFocus = focusedDate.getDate();
    const daysInNewCandidateMonth = new Date(newCandidateDate.getFullYear(), newCandidateDate.getMonth() + 1, 0).getDate();
     if (dayToFocus > daysInNewCandidateMonth) {
        dayToFocus = daysInNewCandidateMonth;
    }
    
    const idealFocusTargetInNewYear = new Date(newCandidateDate.getFullYear(), newCandidateDate.getMonth(), dayToFocus);
    const focusTarget = getFirstFocusableDate(idealFocusTargetInNewYear, minDate, maxDate, disabledDates, offset > 0 ? 1 : -1);

    setFocusedDateState(focusTarget);
    setCurrentMonthDate(new Date(focusTarget.getFullYear(), focusTarget.getMonth(), 1));
  }, [focusedDate, currentMonthDate, currentView, minDate, maxDate, disabledDates]);
  ...
  const handleDayClick = useCallback((day: Date) => {
    ...
    const currentHourForSelection = (value instanceof Date && mode === ChronoPickMode.Single) ? value.getHours() : selectedHour;
    const currentMinuteForSelection = (value instanceof Date && mode === ChronoPickMode.Single) ? value.getMinutes() : selectedMinute;
    
    // If time is enabled, set the time on the selected day
    if (enableTime) {
      finalDay = setTime(day, currentHourForSelection, currentMinuteForSelection);
    }
    ...
    switch (mode) {
      ...
      case ChronoPickMode.Multiple:
        ...
        if (currentDates.some(d => isSameDay(d, finalDay) && (enableTime ? isTimeEqual(d, finalDay): true))) {
          // If already selected, remove it (deselect)
          newSelectedDate = currentDates.filter(d => !(isSameDay(d, finalDay) && (enableTime ? isTimeEqual(d, finalDay): true)));
        } else {
      ...
      case ChronoPickMode.Range:
        const currentRange = (typeof value === 'object' && value ? value : { from: null, to: null }) as DateRange;
        if (!currentRange.from || currentRange.to) { // If no 'from' date or if 'to' is already set (i.e., starting a new range)
          newSelectedDate = { from: finalDay, to: null };
          setTempRangeEnd(null); // Clear any temporary hover state
        } else { // 'from' is set, and 'to' is not (i.e., completing the range)
          if (isSameDay(currentRange.from, finalDay)) { // Clicking 'from' again clears it (or could toggle to single point)
             newSelectedDate = { from: null, to: null }; 
          } else if (isBeforeDay(finalDay, currentRange.from)) { // Ensure 'from' is always before 'to'
  ...
  }, [mode, value, onChange, minDate, maxDate, disabledDates, onVisibilityChange, enableTime, selectedHour, selectedMinute]);
  ...
  const handleMonthSelect = useCallback((monthIndex: number) => {
    ...
    const focusTarget = getFirstFocusableDate(newDate, minDate, maxDate, disabledDates, 0);
  ...
  const handleTimeChange = useCallback((hour?: number, minute?: number) => {
    ...
    if (mode === ChronoPickMode.Single && value instanceof Date) {
        ...
        if(!isDateDisabled(newDateWithNewTime, minDate, maxDate, disabledDates)){
            onChange(newDateWithNewTime);
            setFocusedDate(newDateWithNewTime); 
        }
    } else if (mode === ChronoPickMode.Range && (value as DateRange)?.from && !(value as DateRange)?.to) {
        // If only 'from' is selected in range mode, update 'from' with new time
        const currentFrom = (value as DateRange).from!;
        const newFromDateWithTime = setTime(new Date(currentFrom), newHour, newMinute);
         if(!isDateDisabled(newFromDateWithTime, minDate, maxDate, disabledDates)){
        ...
        const newToDateWithTime = setTime(new Date(currentTo), newHour, newMinute);
        if(!isDateDisabled(newToDateWithTime, minDate, maxDate, disabledDates)){
  ...
  }, [selectedHour, selectedMinute, value, mode, onChange, minDate, maxDate, disabledDates]);
   ...
   */
  const handleKeyDown = (event: React.KeyboardEvent, currentViewArg: CalendarView): boolean => {
    ...
    if (key === 'Enter' || key === ' ') {
      switch (currentViewArg) {
        case CalendarView.Days: handleDayClick(focusedDate); break;
        case CalendarView.Months: handleMonthSelect(focusedDate.getMonth()); break;
        case CalendarView.Years: handleYearSelect(focusedDate.getFullYear()); break;
      }
    ...
    switch (currentViewArg) {
      case CalendarView.Days:
        if (shiftKey) { // Shift key shortcuts
          ...
          switch (key.toLowerCase()) {
            case 'j': // Jan, Jun, Jul cycle
              ...
              break;
            case 'f': targetMonth = 1; break; // February
            ...
            case 'a': // Apr, Aug cycle
              ...
              break;
            case 's': targetMonth = 8; break; // September
            case 'o': targetMonth = 9; break; // October
            case 'n': targetMonth = 10; break; // November
            case 'd': targetMonth = 11; break; // December
            default:
          ...
          if (isLetterShortcut && targetMonth !== null) {
            ...
            switch (key) {
              case 'ArrowLeft': newFocusedDateCandidate = addYears(newFocusedDateCandidate, -1); handled = true; intendedDirection = -1; break;
              case 'ArrowRight': newFocusedDateCandidate = addYears(newFocusedDateCandidate, 1); handled = true; intendedDirection = 1; break;
              case 'ArrowUp': newFocusedDateCandidate = addMonths(newFocusedDateCandidate, -1); handled = true; intendedDirection = -1; break;
              case 'ArrowDown': newFocusedDateCandidate = addMonths(newFocusedDateCandidate, 1); handled = true; intendedDirection = 1; break;
              case 'PageUp': newFocusedDateCandidate = addYears(newFocusedDateCandidate, -1); handled = true; intendedDirection = -1; break; 
              case 'PageDown': newFocusedDateCandidate = addYears(newFocusedDateCandidate, 1); handled = true; intendedDirection = 1; break; 
            }
          }
        } else { // No Shift key: standard day/week/month navigation
          switch (key) {
            case 'ArrowLeft': newFocusedDateCandidate.setDate(newFocusedDateCandidate.getDate() - 1); handled = true; intendedDirection = -1; break;
            case 'ArrowRight': newFocusedDateCandidate.setDate(newFocusedDateCandidate.getDate() + 1); handled = true; intendedDirection = 1; break;
            case 'ArrowUp': newFocusedDateCandidate = addWeeks(newFocusedDateCandidate, -1); handled = true; intendedDirection = -1; break;
            case 'ArrowDown': newFocusedDateCandidate = addWeeks(newFocusedDateCandidate, 1); handled = true; intendedDirection = 1; break;
            case 'PageUp': newFocusedDateCandidate = addMonths(newFocusedDateCandidate, -1); handled = true; intendedDirection = -1; break; 
            case 'PageDown': newFocusedDateCandidate = addMonths(newFocusedDateCandidate, 1); handled = true; intendedDirection = 1; break; 
            case 'Home': newFocusedDateCandidate = startOfWeek(newFocusedDateCandidate); handled = true; intendedDirection = 1; break; // Search forward from start of week
            case 'End': newFocusedDateCandidate = endOfWeek(newFocusedDateCandidate); handled = true; intendedDirection = -1; break; // Search backward from end of week
          }
        }
        break;
      ...
      case CalendarView.Months:
        ...
        switch (key) {
          case 'ArrowLeft': currentMonth--; handled = true; intendedDirection = -1; break;
          case 'ArrowRight': currentMonth++; handled = true; intendedDirection = 1; break;
          case 'ArrowUp': currentMonth -= 3; handled = true; intendedDirection = -1; break; // Move up one row
          case 'ArrowDown': currentMonth += 3; handled = true; intendedDirection = 1; break; // Move down one row
          case 'PageUp': yearForMonthNav--;  handled = true; intendedDirection = -1; break; // Previous year
          case 'PageDown': yearForMonthNav++;  handled = true; intendedDirection = 1; break; // Next year
          case 'Home': currentMonth = 0; handled = true; intendedDirection = 1; break; // January
          case 'End': currentMonth = 11; handled = true; intendedDirection = -1; break; // December
        }
        if (handled) { // Adjust month and year if navigation wrapped around
            if (currentMonth < 0) { currentMonth = 11; yearForMonthNav--; }
            if (currentMonth > 11) { currentMonth = 0; yearForMonthNav++; }
      ...
      case CalendarView.Years:
        ...
        switch (key) {
          case 'ArrowLeft': currentYear--; handled = true; intendedDirection = -1; break;
          case 'ArrowRight': currentYear++; handled = true; intendedDirection = 1; break;
          case 'ArrowUp': currentYear -= 3; handled = true; intendedDirection = -1; break; // Move up one row
          case 'ArrowDown': currentYear += 3; handled = true; intendedDirection = 1; break; // Move down one row
          case 'PageUp': currentYear -= YEARS_PER_VIEW; handled = true; intendedDirection = -1; break; // Previous block of years
          case 'PageDown': currentYear += YEARS_PER_VIEW; handled = true; intendedDirection = 1; break; // Next block of years
          case 'Home': // First year in the current displayed block
            const currentBlockForHome = getYearsRange(newFocusedDateCandidate.getFullYear(), YEARS_PER_VIEW);
            currentYear = currentBlockForHome[0]; 
            handled = true; 
            intendedDirection = 1; 
            break;
          case 'End': // Last year in the current displayed block
            const currentBlockForEnd = getYearsRange(newFocusedDateCandidate.getFullYear(), YEARS_PER_VIEW);
            currentYear = currentBlockForEnd[currentBlockForEnd.length - 1]; 
            handled = true; 
            intendedDirection = -1; 
            break;
        }
        if (handled) { // For Years view, focus is on the year itself (represented by Jan 1st of that year)
    ...
    if (handled) {
      ...
      const trulyFocusableDate = getFirstFocusableDate(newFocusedDateCandidate, minDate, maxDate, disabledDates, intendedDirection);
      setFocusedDateState(trulyFocusableDate); 
      
      // Determine if the calendar view (currentMonthDate) needs to update
      let newCurrentMonthViewAnchor = new Date(trulyFocusableDate.getFullYear(), trulyFocusableDate.getMonth(), 1);
      let needsMonthDateUpdate = false;

      if (currentViewArg === CalendarView.Days) {
        // In Days view, update if month or year of focus changed
        if (newCurrentMonthViewAnchor.getMonth() !== currentMonthDate.getMonth() || newCurrentMonthViewAnchor.getFullYear() !== currentMonthDate.getFullYear()) {
          needsMonthDateUpdate = true;
        }
      } else if (currentViewArg === CalendarView.Months) {
        // In Months view, update if year of focus changed
        if (newCurrentMonthViewAnchor.getFullYear() !== currentMonthDate.getFullYear()) {
          needsMonthDateUpdate = true;
        }
        ...
        const currentYearBlock = getYearsRange(currentMonthDate.getFullYear(), YEARS_PER_VIEW);
        if (trulyFocusableDate.getFullYear() < currentYearBlock[0] || trulyFocusableDate.getFullYear() > currentYearBlock[currentYearBlock.length - 1]) {
          needsMonthDateUpdate = true;
        }
      }
  ...
  const displayValue = useMemo(() => {
    ...
    if (mode === ChronoPickMode.Multiple && Array.isArray(value) && value.length > 0) {
      ...
      return sortedDates.map(d => formatDate(d, effectiveDateFormat, enableTime)).join(', ');
    }
    if (mode === ChronoPickMode.Range && typeof value === 'object' && value) {
      ...
      if (range.from && range.to) {
        ...
        return `${formatDate(fromDate, effectiveDateFormat, enableTime)} - ${formatDate(toDate, effectiveDateFormat, enableTime)}`;
      }
      if (range.from) { // If only 'from' is selected
        return `${formatDate(range.from, effectiveDateFormat, enableTime)} - ...`;
      }
    }
  ...
  const daysToRender = useMemo(() => getDaysInMonth(currentYear, currentMonth), [currentYear, currentMonth]);
  /** Memoized offset for the first day of the `currentMonthDate`'s month (0 for Sunday, etc.). Used for grid layout. */
  const firstDayOffset = useMemo(() => getFirstDayOfMonth(currentYear, currentMonth), [currentYear, currentMonth]);
  /** Memoized array of years to render in the "Years" view, based on `currentMonthDate`'s year. */
  const yearsToRender = useMemo(() => getYearsRange(currentYear, YEARS_PER_VIEW), [currentYear]);
